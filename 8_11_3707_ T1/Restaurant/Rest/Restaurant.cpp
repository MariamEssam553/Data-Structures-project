...
void Restaurant::RunSimulation()
{
	...
	while (!EventsQueue.isEmpty())	//as long as events queue is not empty yet
	{
		...
		Event* ev;
		EventsQueue.peekFront(ev);
		if (ev->getEventTime() == currentTimeStep)
		{
			EventsQueue.dequeue(ev);
			ArrivalEvent* av = dynamic_cast<ArrivalEvent*>(ev);
			CancellationEvent* ce = dynamic_cast<CancellationEvent*>(ev);
			if (av)
			{
				ev->Execute(this);
				numberArrivalEvents++;
...
void Restaurant::Printdata()
{
	...
	std::string numNCooks = std::to_string(numNormalCooks);
	std::string numGCooks = std::to_string(numVeganCooks);
	std::string numVCooks = std::to_string(numVIPCooks);
	std::string numFN = std::to_string(NFTcount);
	std::string numFG = std::to_string(GWTcount);
	std::string numFV = std::to_string(VWTcount);
...
{
	pGUI->PrintMessage("Saving Game, please enter filename: ");
	string fileName;
	fileName=pGUI->GetString() + ".txt";
	ofstream outFile;
...
void Restaurant::PromoteAction(int id,double em)
{
	...
	for(int i=0;i<countNormal; i++)
	{
		if(NormalOrdersArray[i]->GetID() == id)
		{
			indexToDelete=i;
			OrdToDelete=NormalOrdersArray[i];
			check=1;
		}
	}
	if(check==0)
	...
	//Add the order to VIP queue
	OrdToDelete->setType(TYPE_VIP);
	OrdToDelete->setExMony(em);
	OrdToDelete->setTotalMoney(OrdToDelete->GettotalMoney() + em);
	VIPOrders.enqueue(OrdToDelete, OrdToDelete->getPriority());


	//Delete the order from the Normal queue
	for(int i=indexToDelete; i<countNormal; i++)
	{
		NormalOrdersArray[i]=NormalOrdersArray[i+1];
	}

	Order* ord;
	while(!NormalOrders.isEmpty())
	{
		NormalOrders.dequeue(ord);
	}

	for(int i=0;i<countNormal-1;i++)
	{
		NormalOrders.enqueue(NormalOrdersArray[i]);
	}
}
...
//Function responsible for sorting total finished orders to be used in output file
void Restaurant::sortTotalFinishedOrders()
{
	int countNormFinished=0;
	int countVegFinished=0;
	int countVIPFinished=0;

	Order** ArrayToBeSorted;
	int counter=0; //counter of Array to be sorted

	Order** NormFinishedArr=NormalFinished.toArray(countNormFinished);
	Order** VegFinishedArr=VeganFinished.toArray(countVegFinished);
	Order** VIPFinishedArr=VIPFinished.toArray(countVIPFinished);
	int totalCount=countNormFinished+countVegFinished+countVIPFinished;

	//Adding Normal Orders to the array
	for(int i=0;i<countNormFinished;i++)
	{
		ArrayToBeSorted[counter]=NormFinishedArr[i];
		counter++;
	}

	//Adding Vegan Orders to the array
	for(int i=0;i<countVegFinished;i++)
	{
		ArrayToBeSorted[counter]=VegFinishedArr[i];
		counter++;
	}

	//Adding Vip Orders to the array
	for(int i=0;i<countVIPFinished;i++)
	{
		ArrayToBeSorted[counter]=VIPFinishedArr[i];
		counter++;
	}
	...
	for(int i=0;i<counter;i++)
	{
		...
		{
			//Sort by finish time
			if(ArrayToBeSorted[i]->GetFinishTime() > ArrayToBeSorted[j]->GetFinishTime())
			{
			...
			//if they were finished at the same time, sort by service prd
			if(ArrayToBeSorted[i]->GetFinishTime() == ArrayToBeSorted[j]->GetFinishTime())
			{
				if(ArrayToBeSorted[i]->GetServingPrd() > ArrayToBeSorted[j]->GetServingPrd())
				{
					temp=ArrayToBeSorted[i];
					ArrayToBeSorted[i]=ArrayToBeSorted[j];
					ArrayToBeSorted[j]=temp;
	...
	//Now enque the sorted array in the TotalFinished queue
	for(int i=0;i<counter;i++)
	{
		TotalFinished.enqueue(ArrayToBeSorted[i]);
	}
}
